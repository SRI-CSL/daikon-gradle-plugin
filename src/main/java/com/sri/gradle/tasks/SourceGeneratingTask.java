...
public class SourceGeneratingTask extends AbstractNamedTask {
  ...
  public void generateTestDriverCode(){
    ...
    final File testDriverJavaFile = testDriverOutputDir.toPath()
        .resolve(Constants.TEST_DRIVER_CLASSNAME + ".java")
        .toFile();
  ...
  static JavaCode generateTestDriverCode(String testDriverPackage, Collection<String> testClassNames){
    final String joinedTestClasses = Joiner.on(", ").join(
        ImmutableStream.listCopyOf(testClassNames.stream().map(s -> s + ".class")));
  ...
  static List<String> getTestClassNames(Project project, String testDriverPackage){
    ...
    final DirectoryProperty buildDir = project
        .getLayout()
        .getBuildDirectory();
    ...
    final List<File> allTestClasses  = Filefinder.findJavaClasses(testClassesDir, "$"/*exclude those that contain this symbol*/);
