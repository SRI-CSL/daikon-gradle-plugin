...
public class DaikonPlugin implements Plugin<Project> {
  ...
  private CheckForDaikon createCheckForDaikonTask(Project project) {
    ...
    CheckForDaikon checkTask = createCustomPluginTask(project, Constants.CHECK_DAIKON_TASK, CheckForDaikon.class);
    checkTask.setDescription(Constants.CHECK_DAIKON_TASK_DESCRIPTION);
    return checkTask;
  }


  private JavaCompile configureSourceGeneratingTask(Project project, DaikonPluginExtension extension) {
    SourceGeneratingTask genCodeTask =  createCustomPluginTask(
        project, Constants.CODE_GEN_TASK, SourceGeneratingTask.class);
    ...
    Optional<JavaCompile> javaCompile = JavaProjectHelper.findTask(project, JavaPlugin.COMPILE_TEST_JAVA_TASK_NAME, JavaCompile.class);
    ...
    final CompileTestDriverJavaExtension driverExtension = aJavaCompile
        .getExtensions()
    ...
    project.getLogger().debug("Compile test driver separately is " + (driverExtension.getCompileTestDriverSeparately() ? "enabled." : "disabled."));

    final JavaCompile testDriverJavaCompile = JavaProjectHelper.task(project, Constants.COMPILE_TEST_DRIVER, JavaCompile.class);
    testDriverJavaCompile.dependsOn(genCodeTask);
    final CompileTestJavaTaskMutator compileMutator = new CompileTestJavaTaskMutator(project, aJavaCompile.getClasspath());
    ...
    // to use Java lambdas. More info: https://github.com/gradle/gradle/issues/5510#issuecomment-416860213
    genCodeTask.doLast(new Action<Task>() {
  ...
  private static <T extends AbstractNamedTask> T createCustomPluginTask(Project project, String taskName, Class<T> taskClass) {
